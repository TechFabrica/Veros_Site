<section class="parceiro grid__container">
    <?php 
        $image = get_field('parceiro__banner');
        if( !empty( $image ) ):
    ?>

    <img class="parceiro__banner grid__item--4-6" src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />

    <?php endif; ?>
    <div class="parceiro__content-container grid__item--4-6">
        <h3 class="parceiro__title"><?php echo get_field('parceiro__title'); ?></h3>
        <p class="parceiro__content"><?php echo get_field('parceiro__content'); ?></p>
        <div class="parceiro__checklist">
        <?php
            $parceiro_checklist_data = get_field('parceiro__checklist');
            foreach ($parceiro_checklist_data as $checklist):
        ?>
            <span class="parceiro__checklist-item">
                ...
                <?php
                    echo $checklist['parceiro__checklist-item'];
                ?>
            </span>
        <?php
            endforeach;
